...
class Compte_Page < Page
  ...
  def initialize(monApp, header, enciennePage,kolor)

    super("Mon Compte",monApp, :vertical, header, enciennePage,kolor)
    self.spacing = 10
    ...
    tabledozo.attach(Label.new("#{header.retournepseudo()}",kolor, "15"),1,2,0,1)
	...
	#action sur le bouton qui valide le changement de couleur
	 affecter.signal_connect('clicked') {
      ...
      self.supprimeMoi
      menu = Menu_Page.new(@window, @header, self,couleurTitre)
      menu.ajouteMoi
      @window.show_all
    }
      ...
      sonNewMDP = newmdp.text
        ...
        if (File.exists?("Sauvegarde/"+sonPseudo+".marshal")) then
          ...
          if(joueur.mdp.eql?(sonMDP)) then
            ...
            self.supprimeMoi
            menu = Menu_Page.new(monApp, @header, nil,kolor)
            menu.ajouteMoi
            @window.show_all
          else
